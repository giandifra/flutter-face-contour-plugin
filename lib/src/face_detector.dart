...
/// Available face contours detected by [FaceDetector].
    ...
    data['height'],
        ...
        _contours = Map<FaceContourType, FaceContour>.fromIterables(
            ...
            FaceContourType.values.map((FaceContourType type) {
              final List<dynamic> pos = data['contours'][_enumToString(type)];
              return (pos == null)
                  ? null
                  : FaceContour._(
                type,
                pos.map((dynamic offset) => Offset(offset[0], offset[1])).toList(),
              );
            }));
...
///
/// A contour are points on a detected face, such as an eye, nose, or mouth.
class FaceContour {
  FaceContour._(this.type, this.points);
